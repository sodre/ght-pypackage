...
def test_bake_with_defaults(cookies):
    with bake_in_temp_dir(cookies) as result:
        ...
        assert 'elasticdag' in found_toplevel_files
        assert 'tests' in found_toplevel_files
        assert '.cookiecutterrc' in found_toplevel_files
...
@pytest.mark.parametrize('license, target_string', {
    'MIT': 'MIT ',
...
def test_using_pytest(cookies):
    with bake_in_temp_dir(
        cookies,
        extra_context={'project_slug': 'test_using_pytest'}
    ) as result:
        assert result.project.isdir()
        test_file_path = result.project.join(
            'tests/test_test_using_pytest.py'
        )
...
def test_bake_with_no_console_script(cookies):
    context = {'command_line_interface': "None"}
